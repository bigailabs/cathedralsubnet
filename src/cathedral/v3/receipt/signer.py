...
persisted to `$CATHEDRAL_V3_HOME/signer.key`. Override with
`CATHEDRAL_V3_SIGNING_KEY` (hex seed) or `CATHEDRAL_V3_WALLET` (bittensor
...
from cathedral.v3.types import Receipt, Trajectory
...
def load_or_create_signing_key(home: Path) -> SigningKey:
    env_seed = os.environ.get("CATHEDRAL_V3_SIGNING_KEY")
    if env_seed:
