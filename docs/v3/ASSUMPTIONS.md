...
- The `bug_repro` Docker sandbox runner (`cathedral.v3.sandbox`) is **alpha and dev-harness only**. Production `bug_repro` execution belongs in a Cathedral-controlled evaluator service that signs oracle results for validators to verify; this matches the v1 trust flow where validators verify signed Cathedral output rather than executing arbitrary miner-submitted code locally. Tracked in #123. Until the evaluator service ships (Phase 1.5 in `ROADMAP.md`), `bug_repro` stays `OPERATOR_REVIEW` by default and carries no rewardable weight on mainnet.
